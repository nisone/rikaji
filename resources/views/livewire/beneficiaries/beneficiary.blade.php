...
new class extends Component {
    ...
    public function with()
    {
        $id = auth()->user()->id;
        return [
            'beneficiaries' => Beneficiary::search($this->search)->latest()->where('user_id', $id)->paginate(5),
        ];
    }
}; ?>
    ...
    <div class="flex flex-col mx-auto py-4">
        @foreach ($beneficiaries as $b)
            <div class="p-4  flex flex-col space-y-2">
                <a class="text-lg text-blue-500 hover:underline" href={{ route('beneficiaries.user', $b->id) }}>
                    {{ $b->name }}</a>
